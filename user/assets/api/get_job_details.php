...
try {
    ...
    foreach ($attachments as &$attachment) {
        ...
        if (!empty($attachment['picture_path']) && !filter_var($attachment['picture_path'], FILTER_VALIDATE_URL)) {
            ...
            $attachment['picture_path'] = getBaseUrl() . '/' . $cleanPath;
        }
    }
...
function getBaseUrl() {
    ...
    // Get the current request URI and remove the API path and query string
    $requestUri = $_SERVER['REQUEST_URI'];
    $basePath = str_replace('/user/assets/api/get_job_details.php', '', $requestUri);

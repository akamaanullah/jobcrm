...
try {
    ...
    // Get job statistics - Show all jobs (like admin)
    $jobStatsQuery = "
        SELECT 
            COUNT(*) as total_jobs,
    ...
    // Get SLA reminders details (only jobs with SLA within 48 hours) - All jobs
    $slaRemindersQuery = "
        SELECT 
            j.id,
    ...
    // Get recent notifications - All manager notifications
    ...
    $unreadNotificationsQuery = "
        SELECT COUNT(*) as unread_count
        FROM notifications 
        WHERE notify_for = 'manager' AND user_id = :user_id AND is_read = FALSE
    ";

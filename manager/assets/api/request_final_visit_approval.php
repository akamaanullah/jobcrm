...
try {
    ...
    // Create notification for manager (confirmation)
    $managerMessage = "Your final visit approval request for Job #{$job['store_name']} has been submitted successfully.";
    
    $managerNotificationQuery = "
        INSERT INTO notifications (user_id, notify_for, type, job_id, vendor_id, message, is_read, action_required, created_at, updated_at)
        VALUES (:user_id, 'manager', 'final_visit_request', :job_id, :vendor_id, :message, 0, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    ";
    
    $stmt = $pdo->prepare($managerNotificationQuery);
    $stmt->bindParam(':user_id', $userId);
    $stmt->bindParam(':job_id', $jobId);
    $stmt->bindParam(':vendor_id', $vendorId);
    $stmt->bindParam(':message', $managerMessage);
    $stmt->execute();
